...
class MainWindow(QMainWindow):
    ...
    def run_backtest(self):
        ...
        self.backtest_engine.progress_updated.connect(self.update_progress)
    ...
    def generate_detailed_report(self, aggregated_results, symbols, selected_strategies):
        ...
        # Add detailed results for each symbol and strategy
        for symbol in symbols:
            ...
            for strategy_class in selected_strategies:
                ...
                if trades and price_data is not None:
                    # Convert price data to serializable format
                    price_data_dict = {
                        "dates": price_data.index.astype(str).tolist(),
                        "close_prices": price_data['Close'].tolist()
                    }
                    
                    # Convert trade dates to string format
                    serializable_trades = []
                    for trade in trades:
                        serializable_trade = {
