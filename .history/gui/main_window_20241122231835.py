...
class MainWindow(QMainWindow):
    ...
    def run_backtest(self):
        ...
        for symbol in symbols:
            try:
                # Create results table with sorting enabled
                table = QTableWidget()
                ...
                for i, (strategy_name, result) in enumerate(results):
                    ...
                    profit_item = QTableWidgetItem(f"${result['net_profit']:.2f}")
                    if result['net_profit'] > 0:
                        profit_item.setBackground(QColor(0, 150, 0))  # Darker green
                        profit_item.setForeground(QColor(255, 255, 255))  # White text
                    else:
                        profit_item.setBackground(Qt.GlobalColor.red)
                        profit_item.setForeground(QColor(255, 255, 255))
                        
                    table.setItem(i, 1, profit_item)
                    
                    # Make sure numeric values are properly sortable
                    trades_item = QTableWidgetItem()
                    trades_item.setData(Qt.ItemDataRole.DisplayRole, result['total_trades'])
                    table.setItem(i, 2, trades_item)
                    
                    winrate_item = QTableWidgetItem()
                    winrate_item.setData(Qt.ItemDataRole.DisplayRole, result['win_rate'])
                    table.setItem(i, 3, winrate_item)
                    
                    pf_item = QTableWidgetItem()
                    pf_item.setData(Qt.ItemDataRole.DisplayRole, result['profit_factor'])
                    table.setItem(i, 4, pf_item)
                    
                    dd_item = QTableWidgetItem()
                    dd_item.setData(Qt.ItemDataRole.DisplayRole, result['max_drawdown'])
                    table.setItem(i, 5, dd_item)
                    
                    avg_item = QTableWidgetItem()
                    avg_item.setData(Qt.ItemDataRole.DisplayRole, result['avg_trade'])
                    table.setItem(i, 6, avg_item)
    ...
    def create_summary_tab(self, aggregated_results):
        ...
        summary_table.setSortingEnabled(True)  # Enable sorting
        ...
        for i, (strategy_name, results) in enumerate(aggregated_results.items()):
            ...
            net_profit_item.setData(Qt.ItemDataRole.DisplayRole, results['net_profit'])
            net_profit_item.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
            
            if results['net_profit'] > 0:
                net_profit_item.setBackground(QColor(0, 150, 0))  # Darker green
                net_profit_item.setForeground(QColor(255, 255, 255))  # White text
            else:
                net_profit_item.setBackground(Qt.GlobalColor.red)
                net_profit_item.setForeground(QColor(255, 255, 255))
                
            summary_table.setItem(i, 1, net_profit_item)
            
            # Make other numeric columns sortable
            trades_item = QTableWidgetItem()
            trades_item.setData(Qt.ItemDataRole.DisplayRole, results['total_trades'])
            ...
            if results['total_trades'] > 0:
                ...
                win_rate_item.setData(Qt.ItemDataRole.DisplayRole, win_rate)
            else:
                win_rate_item = QTableWidgetItem("N/A")
            summary_table.setItem(i, 3, win_rate_item)
            
            pf_item = QTableWidgetItem()
            pf_item.setData(Qt.ItemDataRole.DisplayRole, results['profit_factor'])
            summary_table.setItem(i, 4, pf_item)
            
            dd_item = QTableWidgetItem()
            dd_item.setData(Qt.ItemDataRole.DisplayRole, results['max_drawdown'])
